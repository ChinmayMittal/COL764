...
int main(int argc, char *argv[]) {
    /*
    CMD: [collection-directory] [index-files] [0|1] [0|1]
        ...
        {0|1} specify the compressions that will be applied – 0 specifies no compression, and 1 specifies variable byte compression.
    ...
    */
    ...
    std::string index_file_name = argv[2];
    int compression_arg = std::stoi(argv[3]);
    int tokenizer_arg = std::stoi(argv[4]);
    ...
    start = std::chrono::high_resolution_clock::now();
    std::ofstream vocab_file(index_file_name + ".dict", std::ofstream::out | std::ofstream::trunc); // normal file
    std::ofstream postings_list_file(index_file_name + ".idx", std::ios::binary | std::ios::trunc); // binary file
